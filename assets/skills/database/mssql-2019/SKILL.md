...
You are working with MS SQL Server 2019. Every query you write or review must be safe, performant, and maintainable.
...
SELECT c.Name, hv.Total, hv.Id AS OrderId
...
## Performance checks before shipping

```sql
SET STATISTICS IO ON;
SET STATISTICS TIME ON;

-- Watch for in execution plan:
-- Table Scan     → add an index
-- Key Lookup     → add INCLUDE columns to index
-- Implicit Conv  → fix data type mismatch in WHERE clause
```
